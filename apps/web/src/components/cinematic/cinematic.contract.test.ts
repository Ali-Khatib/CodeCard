...
describe('Editorial product landing contract', () => {
  ...
  it('walkthrough explains surfaces without dashboard frames', () => {
    ...
    expect(walk).toContain('PRESENT THE WORK.');
    expect(walk).toContain('KEEP PAPERS CLOSE.');
    expect(walk).toContain('CONTEXT, NOT JUST A NAME.');
    expect(walk).toContain('KEEP THE EVENT ATTACHED.');
    expect(walk).toContain('A PRIVATE CIRCLE.');
    expect(walk).toContain('WHAT PEOPLE OPEN.');
    expect(walk).toContain('Open the profile while you talk');
    ...
    expect(walk).toContain('Projects. Research. Circle.');
  ...
  it('keeps a floating glass pill nav and a hero cinema statement reveal', () => {
    ...
    expect(scene).toContain('Present the work.');
    expect(scene).toContain('share your QR code');
    expect(scene).toContain('Save the people you meet.');
    expect(scene).toContain('Keep the context attached.');
