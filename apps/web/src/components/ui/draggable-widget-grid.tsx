...
const Widget = memo(function Widget({
  ...
  const onPointerDown = (e: ReactPointerEvent) => {
    ...
    if ((e.target as HTMLElement).closest('button, input, textarea, select, [data-no-drag]')) {
      return
    }
...
export function DraggableWidgetGrid({
  ...
  const incomingKey = (initialItems ?? DEFAULT_ITEMS).map((item) => `${item.id}:${item.size}`).join('|')
  useEffect(() => {
    const next = initialItems ?? DEFAULT_ITEMS
    setItems((was) =>
      sameOrder(was, next) && was.every((item, i) => item.size === next[i]?.size) ? was : next,
    )
