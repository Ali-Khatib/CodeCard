...
`RATE_LIMITS.auth` is wired to `POST /api/auth/forgot-password` and `POST /api/auth/complete-password-reset`. Browser sign-in and sign-up call Supabase Auth directly and are not Next.js-rate-limited.
