...
| `/api/waitlist` | POST | Public | Zod + 8 KiB + honeypot | N/A | `waitlist` | JSON | Service-role insert; Resend confirmation |
...
| `/api/auth/forgot-password` | POST | Public | Zod email | **same-origin** | `auth` fail-closed | Constant success JSON | Supabase Auth `/auth/v1/recover`; no link in response |
...
- **`/api/auth/oauth-preflight`** — deleted. `startGithubOAuth` stopped calling it
  (the probe aborted healthy authorize flows), leaving an unauthenticated,
  unrate-limited route that made an outbound request. Removed rather than
  hardened, since nothing called it.
